...
async fn transfer_one(
    ...
    // PostgreSQL refuses an explicit value for a GENERATED ALWAYS AS IDENTITY
    // column unless the INSERT opts out. Checked on the TARGET: a cross-dialect
    // copy rewrites AUTO_INCREMENT to GENERATED BY DEFAULT, which needs nothing.
    let overriding: &'static str = if target.dialect() == "postgres"
        && !target
            .list_identity_always_columns(&opts.target_schema, table)
            .await
            .unwrap_or_default()
            .is_empty()
    {
